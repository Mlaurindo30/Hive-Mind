...
class TestPollOnce:
    ...
    def test_conflict_event_triggers_audit(self, monkeypatch):
        from scripts.services.syncthing_watcher import poll_once, _trigger_audit
        ...
        triggered = []
        monkeypatch.setattr("scripts.services.syncthing_watcher._trigger_audit", lambda: triggered.append(1))

        with patch("scripts.services.syncthing_watcher.requests.get", return_value=mock_resp):
            new_id = poll_once(0)
    ...
    def test_non_conflict_event_does_not_trigger_audit(self, monkeypatch):
        from scripts.services.syncthing_watcher import poll_once
    ...
    def test_event_id_advances(self, monkeypatch):
        ...
        monkeypatch.setattr("scripts.services.syncthing_watcher._trigger_audit", lambda: None)
        with patch("scripts.services.syncthing_watcher.requests.get", return_value=mock_resp):
            new_id = poll_once(0)
        assert new_id == 42
    ...
    def test_connection_error_is_swallowed_in_loop(self, monkeypatch):
        ...
        import scripts.services.syncthing_watcher as sw
