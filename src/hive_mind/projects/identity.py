...
class ProjectIdentityResolver:
    ...
    def resolve(
        ...
        *,
        ...
        # Referências (projetos MENCIONADOS na conversa) NÃO ativam o primário:
        # são apenas rastreadas em `referenced_projects` para cross-link. Uma
        # conversa genérica que menciona "Alpha" permanece unclassified/copilot.
        # A única porta de ativação por menção é o semantic_activation opt-in
        # (alta confiança, única referência), logo abaixo.
        if (
            semantic_activation
