...
def install(start: bool, with_tests: bool = False) -> int:
    ...
    if claude_mem_plugin_available():
        ...
        print(
            ...
            "e reexecute o instalador de serviços do Hive-Mind."
        )
...
def _enrich_service_specs(specs: list[dict]) -> list[dict]:
    ...
    command_variants = {
        "sinapse-claude-mem": {"windows": [windows_python, "-m", "hive_mind.services.claude_mem_launcher"], "linux": [str(ROOT / "scripts/services/claude-mem-local.sh")]},
        ...
        "sinapse-graphify-watch": {"windows": [windows_python, "-m", "graphify", "watch", str(ROOT / "cerebro")], "linux": [str(ROOT / "scripts/services/start-watcher.sh")]},
