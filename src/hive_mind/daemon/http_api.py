...
State is read from the daemon state directory, preferring managed ownership
(`services.managed.json`) and falling back to shadow observation
(`services.shadow.json`). Reading is pure; this module never mutates anything.
"""
...
from hive_mind.daemon.state import read_state, service_records, state_ready
...
def create_app(state_dir: Path | str) -> FastAPI:
    ...
    def health() -> dict:
        ...
        services = {
            s["name"]: s.get("readiness", s.get("state", "unknown"))
            for s in service_records(state)
        }
        overall = "healthy" if state_ready(state) else "degraded"
    ...
    def metrics() -> Response:
        ...
        services = service_records(state)
        ready_count = sum(
            1
            for s in services
            if s.get("readiness") == "ready" or s.get("state") == "running"
        )
        lines = [
            ...
            f"hive_daemon_ready {1 if state_ready(state) else 0}",
        ]
