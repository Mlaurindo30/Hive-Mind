# Sinapse Agent — Vault (CLAUDE.md)

> **Claude Code operating manual.** For cross-agent guide, read [[AGENTS.md]].
> Template: [obsidian-mind](https://github.com/breferrari/obsidian-mind) + Graphify + claude-mem + RTK overlay.
> Agent: Thoth (Michel's AI agent, via Hermes framework, WhatsApp interface).

## Memory Architecture (Three Layers)

This vault is the single source of truth. Three complementary systems layer on top:

| Layer | Tool | Purpose | Location |
|-------|------|---------|----------|
| **Structural** | Graphify | Knowledge graph with Leiden clustering (214+ nodes) | `graphify-out/graph.json` |
| **Temporal** | claude-mem | Event tracking, FTS5 full-text search, Chroma semantic | Worker at `:37700` |
| **Execution** | RTK | Shell command optimization (pre-tool-call rewrite) | `~/.hermes/plugins/rtk-rewrite/` |
...
curl -s http://127.0.0.1:37700/health               # worker must respond {"status":"ok"}
systemctl --user is-active sinapse-claude-mem.service
```

## Language & Identity

- **Vault language:** Portuguese (BR) — all notes, templates, and documentation
- **Code & technical output:** English
- **Agent identity:** Thoth — Michel's personal AI agent
- **Agent interface:** WhatsApp (Hermes Agent framework)
- **Company:** THOTH AI
