...
|---|---|---|---|---|---|---|
| C1 | Claude Code (captura nativa) | operational | **PARTIAL** | canário pós-HEAD | `HISTORICALLY_PROVEN`; `FRESH_CHAIN_PENDING` | captura nativa operou antes; exige marcador fresco no staging |
| C2 | Codex | operational | **PARTIAL** | canário pós-HEAD | `HISTORICALLY_PROVEN`; `FRESH_CHAIN_PENDING` | prova automatizada anterior permanece histórica, não é PASS do HEAD atual |
| C3 | Antigravity IDE | operational | **PARTIAL** | canário pós-HEAD | `HISTORICALLY_PROVEN`; `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | marcador IDE real anterior; staging encontra e parseia a fonte real |
| C4 | Antigravity CLI | operational | **PARTIAL** | canário pós-HEAD | `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | não inferir limitação do provider a partir do runtime antigo |
| C5 | Kimi | operational | **PARTIAL** | canário pós-HEAD | `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | `wire.jsonl` real e parser atual; nenhuma conclusão de login/modelo |
| C6 | Qwen CLI | operational | **PARTIAL** | canário pós-HEAD | `HISTORICALLY_PROVEN`; `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | canário anterior estabelece baseline; marcador fresco ainda necessário |
| C7 | Qwen Desktop | operational | **PARTIAL** | canário pós-HEAD | `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | fonte real parseável; cadeia fresca não executada |
| C8 | Hermes (`C:\Users\miche\AppData\Local\hermes\state.db`) | operational | **PARTIAL** | canário pós-HEAD | `HISTORICALLY_PROVEN`; `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | markers/ACKs reais persistidos em `state.db`; staging parseia a fonte correta |
| C9 | Mimo (`C:\Users\miche\.local\share\mimocode\mimocode.db`) | operational | **PARTIAL** | canário pós-HEAD | invocation exit 0; `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | a falha anterior ocorreu na cadeia de captura do Hive-Mind |
| C10 | Kilo (fonte Windows real) | operational | **PARTIAL** | canário pós-HEAD | invocation exit 0; `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | a falha anterior ocorreu na cadeia de captura do Hive-Mind |
| C11 | Copilot | operational | **PARTIAL** | canário pós-HEAD | `HISTORICALLY_PROVEN`; `SOURCE_PARSES_NOW`; `FRESH_CHAIN_PENDING` | execução headless exit 0 e fontes SQLite reais parseadas no staging |
| C12 | OpenClaw (se instalado) | operational | NOT_STARTED | canário quando aplicável | — | ausência atual não encerra nem reprova a capacidade do provider |
| C13 | Roo / Screenpipe / SwarmClaw / demais detectados | operational | NOT_STARTED | canário quando aplicável | — | sem classificação de capacidade baseada apenas em ausência/detecção |
...
| M8 | matriz por provider | operational | **PARTIAL** | `reports/provider-capture-matrix.md` | separa `HISTORICALLY_PROVEN`, `SOURCE_PARSES_NOW` e `FRESH_CHAIN_PENDING`; nenhum PASS fresco no staging | **D004-M correction** |
...
| P1 | política de evidência de providers | processo | **DONE** | `reports/provider-capture-matrix.md` | histórico, parsing atual e cadeia fresca são dimensões independentes; runtime com paths obsoletos é regressão Hive-Mind | D004-M correction |
