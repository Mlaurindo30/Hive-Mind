...
| C2 | Codex | operational | **DONE** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como PASS |
| C3 | Antigravity IDE | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como BLOCKED_BY_PROVIDER |
| C4 | Antigravity CLI | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como BLOCKED_BY_PROVIDER |
| C5 | Kimi | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como FAIL |
...
| C8 | Hermes (`C:\Users\miche\AppData\Local\hermes\state.db`) | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como BLOCKED_BY_PROVIDER |
| C9 | Mimo (`C:\Users\miche\.local\share\mimocode\mimocode.db`) | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como BLOCKED_BY_PROVIDER |
| C10 | Kilo (fonte Windows real a documentar) | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como BLOCKED_BY_PROVIDER |
| C11 | Copilot | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como BLOCKED_BY_PROVIDER |
| C12 | OpenClaw (se instalado) | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como NOT_INSTALLED |
| C13 | Roo / Screenpipe / SwarmClaw / demais detectados | operational | **FAILED** | `hive-mind validate agents` | canário real: ver AUDIT | **D004** — gate de provider. Corrigido em D004-R1: a evidência anterior era mockada (sessão sintética + monkeypatch), viola regra 5 do README; D004-M classificou como NOT_INSTALLED |
...
| AG3 | instalação de instruções | unit+operational | **DONE** | `pytest tests/unit/test_agents_doctor.py::TestInstruction*` + apply em `tmp_path` | 14 passed; bloco gerenciado idempotente, bloco da era PowerShell substituído sem duplicar, conteúdo do usuário preservado | D009-R5; nenhum arquivo de instrução real tocado; evidência: configs temporários |
| AG4 | `agents doctor` | unit+operational | **DONE** | `pytest tests/unit/test_agents_doctor.py::TestDoctor` + `hive-mind agents doctor` no host | 5 passed; diagnóstico real leu configs reais **sem escrever**; `which` injetável impede vazamento do PATH do host | D009-R5; evidência: ambiente real, somente leitura |
| AG5 | wrappers PS1/SH mínimos | unit+integration | **DONE** | `pytest tests/unit/test_registration_wrappers.py tests/integration/test_registration_through_wrappers.py` | 56 + 23 passed; PS1 456→55 L, SH 444→50 L; repasse fiel (ordem, espaços, Unicode, metacaracteres), streams e exit codes preservados; cadeia completa wrapper→CLI→config provada em HOME/APPDATA/project temporários | D009-R6; nenhum config real tocado; evidência: configs temporários |
| AG8 | contrato CLI legado preservado | unit+integration | **DONE** | `hive_mind.agents.compat` + testes acima | `--only/--self/--agent`, posicional, `--check`, `--list`, `--no-instructions`, `--codex-only`, `--claude-only`, `HIVE_SKIP_PROMPT`, exit 2 para agente inválido | D009-R6; tradução em Python, nunca no shell; evidência: configs temporários |
| AG9 | `--check` como gate real | unit+integration | **DONE** | `pytest tests/integration/test_register_mcp_check.py` | 3 passed; exit 0 sse todo provider **detectado** está configurado; provider ausente não é falha; no host real sai 1 e reporta 1 de 9 saudáveis | D009-R6; substitui R5.4 (`--check` sempre 0), que tornava o gate inútil; evidência: ambiente real, somente leitura |
| AG6 | `agents unregister` | unit+operational | **DONE** | `pytest tests/unit/test_agents_doctor.py::TestUnregister` | 5 passed; remove só a entrada `sinapse-memory`, preserva MCP de terceiros em JSON e TOML, idempotente | D009-R5; nenhum config real alterado; evidência: configs temporários |
| AG7 | captura nativa por provider | — | **SUPERSEDED** | — | a captura não é registrada por provider: o caminho canônico é `provider/parser → sessão normalizada → capture_core.ingest → Claude Mem` (ADR-004). Instalar hooks por agente reintroduziria o outbox deprecado | ADR-004; entrega real é D004-R2; evidência: não aplicável por arquitetura |
...
| M14 | identidade canônica chega ao UMC | integration | **DONE** | `pytest tests/integration/test_identity_registry_end_to_end.py` | 16 asserções numa execução: worker real, transporte real, observation real, `workspace_id` = `project_id` decidido antes do POST | ADR-014; registry por `content_session_id` |
| M14c | ingest grava antes de postar | unit | **DONE** | `TestTheDecisionIsWrittenBeforeThePost` | asseverado de dentro do transporte; sem decisão gravada não há POST | M14 |
| M14d | bridge recupera e nunca decide | unit | **DONE** | `TestTheBridgeRecoversAndNeverDecides` | sem `ProjectIdentityResolver`, sem git, sem cwd, sem slug de label, sem ler o formato do id | M14; ADR-014 |
| M14e | degradação sem inventar identidade | unit | **DONE** | `TestDegradation` | `missing_sdk_session`, `missing_capture_identity`, `invalid_capture_identity`, conflito → `QUARANTINED` | M14 |
| M14a | correlação medida do POST até a observation | processo | **DONE** | M14-A, banco temporário de worker real | `contentSessionId` sobrevive via FK declarada `observations.memory_session_id → sdk_sessions.memory_session_id`, com `content_session_id` UNIQUE do outro lado | M14-A; nenhum `UNKNOWN` |
| M14b | mecanismo público do worker para metadata | processo | **FAILED** | M14-A, seção 6 | `metadata` é aceito nos 3 endpoints e escrito como NULL na observation; sem hook, callback ou template | M14-A; opção A descartada por medição |
...
| M3 | outbox backlog ≠ falha de entrega | operational | **DONE** | leitura `mode=ro` dos 2 outboxes | `attempts=0`, `last_error=0`, `dead_letter=0` em 20.649 linhas: nunca houve tentativa. Fila sem dono (ADR-004), não entrega falhada | errata D004-R2; evidência: ambiente real, somente leitura |
...
| BK5 | restore em diretório alternativo | unit+operational | **PARTIAL** | `TestRestore` + prova sintética | 500 registros idênticos, `integrity_check ok`, `foreign_key_check` vazio | dados sintéticos; evidência: dados sintéticos |
...
| R16f | backup restaurável | operational | **PARTIAL** | `TestRestore` + prova sintética (BK5) | 500 registros idênticos, `integrity_check ok`, `foreign_key_check` vazio — em banco sintético | restauração de banco **real** continua NOT_EXECUTED (BK7); evidência: dados sintéticos |
