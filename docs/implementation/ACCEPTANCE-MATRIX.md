...
| M1 | Claude Mem recebe project canônico | unit+integration | **DONE_TEMP_STORE** | `pytest tests/integration/test_capture_canonical_delivery_proof.py` | 12 passed sobre evento novo `HM-D004-R2-*`; `project` entregue = `project_name` canônico; rótulo livre só em `metadata.capture.raw_project_label` | D004-R2; POST ao worker é **gravado, não executado** — a linha na store viva não é provada |
| M2 | bridge workspace_id=project_id | integration | **DONE_TEMP_STORE** | `test_the_bridge_carries_project_id_into_umc_workspace_id` | bridge de **produção** contra `CLAUDE_MEM_DB` e `SINAPSE_HOME` temporários; `workspace_id` gravado = `project_id` canônico, `identity_status=canonical` | D004-R2 |
...
| M8 | matriz por provider | operational | **NOT_STARTED** | — | um evento verde prova o caminho, não os 13 providers | D004 (próxima etapa) |
