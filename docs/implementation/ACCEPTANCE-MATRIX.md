...
| M1 | Claude Mem recebe project canônico | integration | **PARTIAL** | `pytest tests/integration/test_capture_isolated_worker_delivery.py` | 14 passed; worker real descartável, transporte real, `user_prompt` gravado, log do worker registra `Session initialized {project=Hive-Mind}`; rótulo livre ausente do log | D004-R2W; **observation não provada** — ambiente isolado não tem provider |
