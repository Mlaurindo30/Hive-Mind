...
> **Versão**: 2.7 (P1+P2 implementados — timers reproduzíveis + M9 instrumentado; Fase 2 daily/weekly/topics LIVE)
> **Data**: 2026-06-17
...
> **DESENHADA p/ implementação** (segue §14: cada task tem O QUÊ / ONDE / ASSINATURA /
> COMO / VALIDAÇÃO). Não começar sem P1+P2 (§14.4) e sem reler §14.1–14.2. Todo script
> nasce INERTE (R9) e validado contra o vault/DB reais (R1) com teste real (R5).
> **Atenção ao schema real (§14.2)**: NÃO existe `backlinks_count`, `last_reviewed_at`
> nem tabela `merge_log` — use as colunas reais (`updated_at`, `visibility`, `metadata`,
> `type`) e derive o resto do `source_file`/frontmatter.
...
#### P3 — Fase 3 (síntese viva) — **DESENHADA em §11/Fase 3 (F3.1-F3.4)**
...
| Fase 3 | ❌ proposta (desenhada §11) | **P3** (P1+P2 ✅ — desbloqueada) |

---

*Documento vivo. Versão 2.7 (P1 timers reproduzíveis `999db80` + P2 M9 `e06a2d4`; Fase 2 daily/weekly/topics LIVE, dream gated). Próxima revisão: após 7d de M9 verde → habilitar dream, ou início da Fase 3.*
