...
> **Versão**: 2.23 (Fase 5 desenhada: alert_dispatcher + HNSW wiring + dashboard Dataview; M13; 377 testes)
> **Data**: 2026-06-19
...
#### F4.1 — `decision_promoter.py` → `cortex/frontal/decisoes/` — ✅ FEITO (`66fee55`)
...
#### F4.2 — `project_synthesizer.py` → `cortex/frontal/projetos/` — ✅ FEITO (`84486e4`)
...
#### F4.3 — `pattern_distiller.py` → `cerebelo/padroes/` (memória procedural) — ✅ FEITO (`959fdb5`)
...
#### F4.4 — `conflict_detector.py` → `cortex/insula/conflitos/` — ✅ FEITO (`b5b3d66`)
...
#### F4.5 — `work_tracker.py` → `cortex/frontal/trabalho/ativo/` (opcional) — ✅ FEITO (`f1878cb`)
...
### Fase 5 — Observabilidade Ativa (alertas + busca semântica) — 🔵 DESENHADA

> **MOTIVAÇÃO**: o sistema hoje **escreve mas não fala**. Snapshots de saúde existem
> em `insula/saude/`, métricas M1-M12 são calculadas, alertas são avaliados — mas
> nada chega ao usuário sem que ele abra o arquivo. A Fase 5 fecha o **loop de
> feedback**: alertas chegam ao inbox (onde o usuário já processa entradas) e o
> vault vira consultável semanticamente a partir de qualquer sessão.
>
> **Pré-condição**: Fase 4 ✅ LIVE e M10-M12 calculados (F4.6). Seguir §14 (R1-R10).

#### F5.1 — `alert_dispatcher.py` → `cortex/parietal/inbox/` (alertas ativos)

- **O QUÊ**: lê o snapshot do dia em `insula/saude/YYYY-MM-DD.md`, extrai cada alerta
  ativo (seção `## Alertas`) e escreve uma nota por alerta em `parietal/inbox/YYYY/MM/DD/`
  — a caixa de entrada natural do sistema (§2, "sensorial"). O usuário processa no
  mesmo lugar que processa qualquer captura.
- **ONDE**: novo `scripts/alert_dispatcher.py`; `core.paths.SAUDE_ROOT`, `INBOX_ROOT`.
  Read-only no snapshot; escreve APENAS em `inbox/`. Sem LLM.
- **ASSINATURA**:
  ```python
  def load_alerts(saude_root: Path, *, date: date = None) -> list[str]
      """Extrai lista de strings de alerta do snapshot do dia (seção ## Alertas)."""
  def dispatch(alerts: list[str], inbox_root: Path, *, dry_run: bool = True,
               now: datetime = None) -> list[Path]
      """Escreve 1 nota por alerta em inbox/{YYYY/MM/DD}/alerta-{HHMMSS}-{code}.md.
      Idempotente: não reescreve se já existe nota com mesmo conteúdo (hash)."""
  def main() -> int   # argparse --apply (default dry-run)
  ```
- **Template de nota de alerta** (frontmatter):
  ```yaml
  ---
  type: health-alert
  date: 2026-06-20
  ...
  message: "M4: órfãos em 18% (> 15%)."
  suggested_action: "Rodar audit_memory.py --fix-orphans ou revisar MOCs."
  auto_resolved: false
  ---
  ```
- **Idempotência**: nome do arquivo = `alerta-{HHMMSS}-{content_hash8}.md`; se o hash
  já existe na pasta do dia, não recria (evita duplicatas em re-execuções).
- **Métrica nova M13**: `alerts_dispatched_today` = contagem de `alerta-*.md` em
  `inbox/YYYY/MM/DD/` com `type: health-alert`. Adicionada ao `health_dashboard`.
- **CADÊNCIA**: `sinapse-alert.timer` (diário, pós `sinapse-health.timer`). Read-only
  no snapshot → seguro no `enabled`.

#### F5.2 — Busca semântica HNSW via MCP `search_memories`

- **O QUÊ**: o HNSW index em `core/hnsw_index.py` existe com "wiring parcial" (§1.2).
  A tool MCP `search_memories` hoje usa full-text (`neurons.content LIKE ?`). Wirear
  o HNSW para que `search_memories` retorne os **top-k neurônios por cosine**,
  tornando o vault consultável semanticamente de qualquer sessão (Claude, Codex, etc.).
- **ONDE**: `scripts/sinapse-mcp.py` (tool `search_memories`); `core/hnsw_index.py`
  (já existe); `core/paths.HNSW_INDEX` (confirmar constante ou adicionar).
- **ASSINATURA** (mudança na tool MCP, não no hnsw_index):
  ```python
  # sinapse-mcp.py — search_memories handler
  def search_memories(query: str, *, top_k: int = 10, project: str = None,
                      mode: str = "semantic") -> list[dict]:
      """mode='semantic': HNSW cosine; mode='text': full-text (legado).
      Retorna [{label, source_file, project, topic, score, aliases}]."""
  ```
- **COMO**: embed `query` com fastembed (mesmo modelo do dedup existente) → busca
  HNSW → resolve `source_file` → retorna metadados. Fallback para full-text se
  HNSW index não existe ou está desatualizado (idade > 7d).
- **Índice**: `core/hnsw_index.py` já tem `rebuild()` e `search()`; verificar se
  `sinapse-audit.timer` (Sun 05:00) já chama `rebuild()` — se não, adicionar.
- **VALIDAÇÃO**: query semântica retorna neurônio relevante que full-text perderia
  (sinônimo / paráfrase); fallback funciona quando index ausente.
- **CADÊNCIA**: index rebuiltado pelo `sinapse-audit.timer` existente (sem novo timer).

#### F5.3 — Dashboard Dataview em `cortex/insula/saude/_Dashboard.md` (opcional)

- **O QUÊ**: nota Obsidian com blocos **Dataview JS** que agregam M1-M12 de todos os
  arquivos `insula/saude/YYYY-MM-DD.md` — trends dos últimos 7/30/90 dias, renderizados
  pelo plugin Dataview em tempo real no Obsidian. Não depende de scripts Python.
- **ONDE**: arquivo estático `cerebro/cortex/insula/saude/_Dashboard.md` (gerado uma
  vez por `scripts/health_dashboard.py --generate-dashboard`). Sem timer.
- **COMO**: cada bloco Dataview JS lê os snapshots como `LIST` ou `TABLE` e extrai
  o valor da métrica via regex no body (ex.: `| M1 átomos/dia | {n} |`). O resultado
  é uma série temporal visual dentro do Obsidian, zero infra.
- **VALIDAÇÃO**: abrir no Obsidian com plugin Dataview ativo → tabela de tendências visível.
- **Prioridade**: `nice-to-have` — os dashboards estáticos de `insula/saude/` já existem;
  este é upgrade visual.

**Entregáveis Fase 5**: 1 script novo (`alert_dispatcher`) + 1 wiring MCP (`search_memories`)
+ 1 arquivo de dashboard (opcional) + 1 nova métrica (M13) + 1 novo timer (`sinapse-alert`).
**Pré-condições**: F4.6 ✅ (M10-M12 calculados; `sinapse-health.timer` ativo).
...
| Fase 4 | 2-3 sprints | LLM rende pouco s/ OAuth; mutar frontal sem revisão | ✅ **LIVE** (F4.0-F4.6; 377 testes) |
| Fase 5 | 1-2 sprints (W26-W27) | HNSW index desatualizado; alertas duplicados | 🔵 **DESENHADA** (alert_dispatcher + HNSW wiring + dashboard) |

**Total**: 10-12 sprints (~5-6 meses). Fase 0 consumida em 1 sessão (incremento contínuo, não sprint formal).
...
#### P4 — Fase 4 (auto-preenchimento do frontal + padrões + conflitos) — ✅ LIVE

Implementada task-a-task em **§11 → Fase 4** (F4.0-F4.6, commits `5c18f55`→`3cd0e09`).
F4.6 (M10/M11/M12) completo em 2026-06-19; 377 testes verdes.

#### P5 — Fase 5 (observabilidade ativa + busca semântica) — 🔵 DESENHADA em §11/Fase 5

Desenhada task-a-task em **§11 → Fase 5** (F5.1-F5.3). Ordem recomendada:
**F5.1 (alert_dispatcher) primeiro** — menor risco, fecha o loop de feedback; depois
**F5.2 (HNSW wiring)** — maior impacto (semantic search de qualquer sessão); **F5.3**
(dashboard Dataview) é nice-to-have, pode ser paralelizado. Cada script INERTE até
validado (R1) + testado (R5). **Pré-condição**: F4.6 ✅ LIVE.
...
| Fase 5 (alert_dispatcher + HNSW wiring + dashboard) | 🔵 **DESENHADA** | F5.1-F5.3 em §11/Fase 5; pré-condição F4.6 ✅ |

---

*Documento vivo. Versão 2.23 (Fase 5 desenhada: F5.1 `alert_dispatcher` → inbox, F5.2 HNSW wiring no MCP `search_memories`, F5.3 Dashboard Dataview; nova métrica M13 `alerts_dispatched_today`; 377 testes).*
