...
### ADR-019 — Model Gateway as the canonical LLM execution layer (Priority 1, unificado)

**Decision:** `core/model_gateway.py` + `core/model_registry.py` are the **only** LLM execution path in the Hive-Mind. `core/llm_client.call_llm_with_fallback` is a thin wrapper that delegates to `ModelGateway.from_combined_config()` by default; the legacy `_legacy_call_llm_with_fallback` is reachable only via `HIVE_FORCE_LEGACY_LLM=true` (emergency bypass) or when `image_path` is passed (vision bridge). The registry composes `core.auth.PROVIDERS_CONFIG` + the `.env` `HIVE_{ROLE}_PROVIDER/MODEL/FALLBACK*` and applies `config/model-gateway.yaml` as an OPTIONAL override layer (never requiring `provider`/`model` per role). `MODEL_GATEWAY_MODE=auto` (default) uses the gateway and falls back to legacy on failure with an explicit warning + telemetry; `MODEL_GATEWAY_MODE=on` makes the gateway mandatory; `MODEL_GATEWAY_ENABLED` is a deprecated shim. See [`14-model-gateway.md`](14-model-gateway.md) and `specs/model-gateway-unification.md`.
**Rationale:** a single source of truth for LLM execution eliminates the dual-path confusion where the gateway coexisted as an opt-in layer next to the legacy code. New inference backends are addable via `config/model-gateway.yaml` overrides, not by touching call sites; explicit fallback (never silent success) and redacted telemetry keep the same operational guarantees as the legacy path.
**Trade-off:** the registry unifies three config sources; the override model means a small migration surface for operators who were pinning `roles.<name>.provider` in YAML. Mitigated by `ModelRegistry.validate()` reporting `unsupported_explicit` for any provider that the registry cannot map, and by `setup-brain.py` printing the resolved table after every save.
