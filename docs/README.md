...
> **Status:** Fase HM-12 (Federated Swarm) entregue · **Fase K (Conhecimento Born-Large)**: K0–K10 implementadas e revalidadas em local-full · **Testes:** 1032 funções `test_` em 172 arquivos com testes (contagem 2026-07-09; era 706/123 em 2026-07-01 — meça de novo com `rg -n "^\s*(async\s+def|def)\s+test_" tests | wc -l`, não confie neste número parado); gate K9 real separado em `tests/run_real_knowledge.sh` com 59/59 passed, 0 skipped
...
| 14 | [Model Gateway (canonical LLM execution layer)](14-model-gateway.md) | Priority 1 (unificado) — única API de execução de LLM, roteamento por role/capability, adapters OpenAI-compatible (LM Studio, llama.cpp, vLLM, SGLang, LiteLLM), fallback explícito, `MODEL_GATEWAY_MODE=auto` por padrão, `HIVE_FORCE_LEGACY_LLM=true` como bypass emergencial | Engenharia / ML Engineers |

> Todos os documentos 01–07 foram reescritos para v3.0.0 em 2026-06-13. Em 2026-06-30, a frente de Conhecimento Born-Large (K0–K10) foi consolidada diretamente no canônico: **`01-architecture.md` §22–§31** é hoje a referência normativa completa (fluxo de promoção, tipos canônicos, chunks, vector search, DocumentPipeline, RetrievalRouter, métricas, contratos evolutivos).
>
> **Nota de manutenção (2026-07-09):** os documentos `06-gap-analysis.md`, `10-implementation-roadmap.md`, `11-knowledge-promotion-architecture.md`, `12-knowledge-implementation-plan.md` e `13-*.md` são citados em vários lugares deste projeto (README raiz, specs/, integrações) como referências separadas, mas essa numeração **não existe neste checkout de `docs/`** — o conteúdo foi consolidado em `01-architecture.md` (ver acima) ou nunca foi versionado. Trate citações a esses caminhos como apontando para `01-architecture.md §22–§31` até que os arquivos separados sejam recriados (ou as citações restantes, removidas) intencionalmente.

## Relatórios e planos

> `docs/plans/2026-06-10-auditoria-tecnica-completa.md` e `docs/walkthrough.md` são citados a partir de outros documentos mas não existem neste checkout — provavelmente vivem apenas no vault pessoal (`cerebro/`) ou nunca foram versionados. Não recrie o conteúdo por suposição; confirme com quem escreveu a auditoria original antes de linkar de volta a eles.
...
| [`../config/sinapse.yaml`](../config/sinapse.yaml) | Configuração central comentada |
| [`../tests/README.md`](../tests/README.md) | Estrutura e convenções da suíte de testes |

> `PROJECT_STATUS.md` e `IMPLEMENTATION.md` são citados como acompanhamento de fases / log de entregas por data, mas não existem na raiz deste repositório. O acompanhamento de fases equivalente hoje vive em `01-architecture.md §21` ("Phase Governance").
...
Plano de execução:   01-architecture.md §21 (Phase Governance) → fases K0-K10, modelos/env, vendors e testes reais
...
6. **Entendendo o pipeline de conhecimento (K0–K10):** [01-architecture.md §22–§31](01-architecture.md#22-arquitetura-de-conhecimento-born-large) (referência normativa completa; ver nota de manutenção acima sobre `docs/11`/`docs/12`)
